...
def manager_dashboard(request):
    ...
    withdrawals_today = Transaction.objects.filter(tx_type='WITHDRAWAL', created_at__gte=today)
    withdrawals_today_val = withdrawals_today.aggregate(Sum('amount'))['amount__sum'] or 0
    
    # Alertes Système (Dynamiques basées sur l'état réel)
    pending_withdrawals_count = pending_txs.filter(tx_type='WITHDRAWAL').count()
    alerts = []
    ...
    if failed_txs_today > 0:
        alerts.append({'type': 'error', 'message': f'{failed_txs_today} transaction(s) échouée(s) aujourd\'hui.'})
    alerts.append({'type': 'success', 'message': 'Le système de paiement est opérationnel et synchronisé.'})
...
def process_transaction(request, tx_id, action):
    ...
    if action == 'approve':
        ...
        if tx.tx_type == 'DEPOSIT':
            ...
            tx.user.save()
        elif tx.tx_type == 'PAY_INVEST' and tx.related_investment:
            from django.utils import timezone
            ...
            if is_first_investment and tx.user.sponsor and inv.amount_invested >= 10000:
                ...
                Notification.objects.create(
                    ...
                    message=f"Votre filleul {tx.user.username} a activé un palier VIP. Vous gagnez +20 Points VIP !"
                )
            Notification.objects.create(
                user=tx.user, title="Palier Activé ✅",
                message=f"Votre paiement de {tx.amount:,.0f} XOF a été confirmé. Vote Palier est Actif !".replace(',', ' ')
            )
            
        # Pour les retraits, le solde a DEJA été déduit du client lors de sa demande
        tx.save()
        messages.success(request, f"Transaction de {tx.amount} XOF validée avec succès.")
        ...
        tx.save()
        messages.info(request, f"Transaction rejetée. Les fonds liés ont été restitués si c'était un retrait.")
...
def superadmin_dashboard(request):
    ...
    completed_deposits = Transaction.objects.filter(tx_type='PAY_INVEST', status='SUCCESS').aggregate(Sum('amount'))['amount__sum'] or 0
    completed_withdrawals = Transaction.objects.filter(tx_type='WITHDRAWAL', status='SUCCESS').aggregate(Sum('amount'))['amount__sum'] or 0
    ...
    total_users = User.objects.count()
    users_with_deposit = Transaction.objects.filter(tx_type='PAY_INVEST', status='SUCCESS').values('user').distinct().count()
    ...
    for d in last_7_days_dates:
        ...
        dep_sum = Transaction.objects.filter(tx_type='PAY_INVEST', status='SUCCESS', created_at__gte=day_start, created_at__lt=day_end).aggregate(Sum('amount'))['amount__sum'] or 0
        with_sum = Transaction.objects.filter(tx_type='WITHDRAWAL', status='SUCCESS', created_at__gte=day_start, created_at__lt=day_end).aggregate(Sum('amount'))['amount__sum'] or 0
...
def admin_settings_view(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            AuditLog.objects.create(admin_user=request.user, action="A modifié les paramètres de la plateforme", severity='WARNING')
            return redirect('/superadmin/')
    else:
        form = SystemSettingsForm(instance=settings_obj)
    return render(request, 'superadmin/settings.html', {'form': form, 'title': 'Paramètres Plateforme'})
...
def admin_payment_config_view(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            p = form.save()
            AuditLog.objects.create(admin_user=request.user, action=f"A configuré l'API Mobile Money: {p.provider_name}", severity='WARNING')
...
def admin_user_toggle(request, user_id):
    ...
    if not u.is_superuser: # Protéger les super admins
        u.is_active = not u.is_active
        u.save()
        AuditLog.objects.create(admin_user=request.user, action=f"A {'débloqué' if u.is_active else 'bloqué'} l'utilisateur {u.username}", severity='WARNING')
...
def admin_user_create_view(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            u = form.save()
            AuditLog.objects.create(admin_user=request.user, action=f"A créé l'utilisateur {u.username}", severity='INFO')
            return redirect('admin_users')
    else:
        form = CustomUserCreationForm()
        # Modifions les widgets du formulaire par défaut pour qu'ils respectent le design sombre
        for field_name, field in form.fields.items():
            field.widget.attrs['class'] = 'form-input'

    return render(request, 'superadmin/user_form.html', {'form': form, 'title': 'Créer un Utilisateur', 'is_create': True})
...
def admin_user_edit_view(request, user_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            AuditLog.objects.create(admin_user=request.user, action=f"A modifié l'utilisateur {u.username}", severity='WARNING')
            return redirect('admin_users')
    else:
...
def admin_user_delete_view(request, user_id):
    ...
    if not u.is_superuser and request.method == 'POST':
        username = u.username
        u.delete()
        AuditLog.objects.create(admin_user=request.user, action=f"A supprimé définitivement l'utilisateur {username}", severity='CRITICAL')
    return redirect('admin_users')
...
def admin_product_toggle(request, prod_id):
    ...
    p.save()
    AuditLog.objects.create(admin_user=request.user, action=f"A {'activé' if p.is_active else 'désactivé'} le palier {p.name}", severity='WARNING')
